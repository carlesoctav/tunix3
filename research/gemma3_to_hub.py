#!/usr/bin/env python3
"""
Export a Gemma-3 model with merged LoRA weights to SafeTensors and optionally
...
Typical usage:
    python research/gemma3_to_hub.py \
      --checkpoint-dir "gs://your-bucket/tunix/gemma3-repro/actor" \
      --output-dir "./exported_gemma3_lora" \
      --hf-repo-id "username/gemma3-4b-lora" \
      --step 1000 \
      --lora 16
"""
...
def download_hf_model(hf_model_id: str, cache_dir: str) -> str:
    ...
    Args:
        hf_model_id: HuggingFace model ID (e.g., 'google/gemma-3-4b-it')
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Export Gemma-3 with merged LoRA weights to HuggingFace format"
    )
    ...
    p.add_argument(
        ...
        help="Model name (e.g., gemma3-1b-it)",
    )
    p.add_argument(
        "--model-id",
        type=str,
        default="google/gemma-3/flax/gemma3-1b-it",
    ...
    p.add_argument(
        ...
        type=int,
        default=16,
        help="LoRA rank and alpha (default: 16). Sets both rank and alpha to this value.",
    )
    p.add_argument(
        "--lora-alpha",
        type=float,
        default=None,
        help="LoRA alpha (default: equals --lora value)",
    )
    p.add_argument(
        "--module-path",
        type=str,
...
def main():
    ...
    print("=" * 60)
    print("Gemma-3 LoRA Export Tool (Using safetensors_saver)")
    print("=" * 60)
    print("Configuration:")
    ...
    model_config = {
        ...
        "model_id": gemma3_params.GEMMA3_1B_IT if args.model_name == "gemma3-1b-it" else args.model_id, # Use GCS path for 1b-it
        "model_source": "gcs", # Force GCS for Gemma3 base model loading
    ...
    # LoRA config
    # If alpha is not specified, default it to the rank (value of --lora)
    ...
    if step is None:
        ...
        print(f"No step specified, using latest step: {step}")
    else:
        print(f"Using specified step: {step}")
